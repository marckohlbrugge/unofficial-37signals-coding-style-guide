...
Safari freezes on complex nested `:has()` selectors (#1089).
...
### N+1 → JOINs
Replace `find_each` loops with JOINs for bulk operations (#1129).
...
- Reduce if initial render is slow (#1089: 50 → 25)
...
### Read Replicas
Use `preprocessed: true` - lazy generation fails on read-only replicas (#767)

### Slow Uploads
Extend signed URL expiry from default 5 min to 48 hours (#773).
Cloudflare buffering can exceed default timeout.

### Large Files
Skip previews above size threshold (e.g., 16MB) to avoid timeouts (#941)
...
### Debouncing
100ms debounce on filter search feels responsive (#567)
